...
const badgeStyles: Record<BadgeVariant, string> = {
  ...
  outline: 'text-gray-950 dark:text-gray-50 border',
...
const Badge: React.FC<BadgeProps> = ({ className, variant = 'default', ...props }) => {
  return (
    <div
      className={`inline-flex items-center rounded-full border border-gray-200 dark:border-gray-800 px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-gray-950 focus:ring-offset-2 ${badgeStyles[variant]} ${className}`}
      {...props}
    />
  );
};
