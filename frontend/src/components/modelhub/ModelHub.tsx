...
const ModelHub: React.FC<ModelHubProps> = ({ models, onDownloadModel, onSelectModel, selectedModel }) => {
  ...
  const ModelCard: React.FC<{ model: Model }> = ({ model }) => (
    <Card className="hover:shadow-lg transition-shadow bg-white dark:bg-gray-950 flex flex-col">
      ...
      <div className="flex-1 p-6 overflow-hidden">
        <Tabs defaultValue="cortex" className="w-full h-full flex flex-col">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="cortex" className="flex items-center gap-2"><Flame className="h-4 w-4" /> Cortex Models</TabsTrigger>
            <TabsTrigger value="local" className="flex items-center gap-2"><HardDrive className="h-4 w-4" /> Local Models</TabsTrigger>
            <TabsTrigger value="cloud" className="flex items-center gap-2"><Cloud className="h-4 w-4" /> Cloud Models</TabsTrigger>
          </TabsList>
          <div className="flex-grow mt-2">
            <TabsContent value="cortex" className="h-full">{renderTabContent("Cortex's Custom Models", "Optimized models built specifically for Cortex's local AI experience", cortexModels)}</TabsContent>
            <TabsContent value="local" className="h-full">{renderTabContent("Open Source Models", "Community models from HuggingFace that run locally on your device", localModels)}</TabsContent>
            <TabsContent value="cloud" className="h-full">{renderTabContent("Cloud Providers", "Connect your API keys to use cloud-based AI models", cloudModels)}</TabsContent>
          </div>
        </Tabs>
      </div>
    </div>
